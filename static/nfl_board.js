...
function buildApiUrl(refresh = false) {
  const url = new URL("/api/nfl/daily", window.location.origin);
  if (boardMode === "demo") {
...
function renderSlate(slate, edgeFraction = 0.08) {
  ...
  for (const game of slate) {
    ...
    const preTag = game.game_type === "preseason" ? " (pre)" : "";
    const spreadLine =
      game.home_spread_point != null
        ? `${game.home_team} ${fmtSpread(game.home_spread_point)}`
        : "—";
    const matchup = game.game_id
      ? `<a href="/nfl/game/${encodeURIComponent(game.game_id)}">${game.matchup}${preTag}</a>`
      : `${game.matchup}${preTag}`;
    tr.innerHTML = `
      <td>${matchup}</td>
...
async function loadBoard(refresh = false) {
  ...
  els.loadingMessage.textContent =
    boardMode === "demo" ? "Loading demo NFL board…" : "Loading NFL board…";
