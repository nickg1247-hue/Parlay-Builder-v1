/** NFL game detail — live scores + ML / spread / O/U model and markets. */
...
(function () {
  ...
  const scoresUrl = dateParam
    ? `/api/scores/today?sport=nfl&date=${encodeURIComponent(dateParam)}`
  ...
  function insightsUrl(refresh) {
    ...
    return `/api/games/nfl/${encodeURIComponent(gameId)}/insights${q ? `?${q}` : ""}`;
  }
