...
**Recommend score (ERVA engine):** modular `app/services/fantasy_draft/` — hard eligibility via `can_team_draft_player` / `get_eligible_players` before scoring. Score prioritizes **team usefulness** and live roster need over raw projection: VORP (WRT demand included), scarcity, tier drop, roster need + starter-need urgency (updates every pick from current lineup), lineup/flex impact, ADP, next-pick availability, upside, risk, imbalance, bye, look-ahead. Projection is soft-capped when it does not fill an open hole. `projected_role` is STARTER / WRT / BENCH. Weights in `DEFAULT_DRAFT_WEIGHTS`.
