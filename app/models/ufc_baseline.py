"""UFC home_fighter_win baseline: logistic + Platt calibration + Elo comparison."""
...
MODEL_ARTIFACT = PROJECT_ROOT / "data" / "processed" / "ufc_baseline_model.joblib"
METRICS_JSON = PROJECT_ROOT / "data" / "processed" / "ufc_baseline_metrics.json"
MODELS_DIR = PROJECT_ROOT / "data" / "processed" / "models"
ACTIVE_UFC_MANIFEST = PROJECT_ROOT / "data" / "processed" / "active_ufc_model.json"
PARQUET_PATH = PROJECT_ROOT / "data" / "processed" / "ufc_fights.parquet"

BASE_TRAIN_SEASONS = (2021, 2022)
PLATT_SEASON = 2023
HOLDOUT_SEASON = 2024
FEATURE_SET = "ufc_v1"
...
def load_fights() -> pd.DataFrame:
    if PARQUET_PATH.exists():
        ...
        conn = get_connection()
        try:
            df = pd.read_sql("SELECT * FROM ufc_fights ORDER BY date, fight_id", conn)
        finally:
            conn.close()
    if df.empty:
        raise FileNotFoundError(
            f"No UFC fights at {PARQUET_PATH}. Run scripts/bootstrap_ufc.py first."
        )
...
def _elo_update(home_elo: float, away_elo: float, home_win: int) -> tuple[float, float]:
...
def current_elo_ratings(history: pd.DataFrame) -> dict[str, float]:
    ratings: dict[str, float] = {}
    hist = history.sort_values(["date", "fight_id"]).reset_index(drop=True)
...
def attach_elo_for_slate(
    ...
    *,
    ...
    del fights_df
    hist = history if history is not None else load_fights()
    out = df.copy()
...
def train_logistic(train: pd.DataFrame) -> Pipeline:
    ...
    model.fit(train[FEATURE_COLUMNS].values, train["home_win"].values)
    return model


def load_model_artifact() -> dict[str, Any]:
    if ACTIVE_UFC_MANIFEST.exists():
        manifest = json.loads(ACTIVE_UFC_MANIFEST.read_text(encoding="utf-8"))
    ...
    raise FileNotFoundError(
        f"No UFC model at {MODEL_ARTIFACT}. Run scripts/bootstrap_ufc.py first."
    )
...
def predict_home_win_proba(df: pd.DataFrame) -> np.ndarray:
    ...
    if len(prepared) != len(df):
        by_id = dict(zip(prepared["fight_id"].astype(str), raw))
        return df["fight_id"].astype(str).map(by_id).to_numpy()
    return raw
...
def run_training() -> dict[str, Any]:
    ...
    rest_fill = float(pd.concat([base["home_rest_days"], base["away_rest_days"]]).median())
    ...
    raw_platt = base_model.predict_proba(platt_df[FEATURE_COLUMNS].values)[:, 1]
    ...
    model_metrics = compute_metrics("logistic_v1", y_holdout, cal_holdout)
    home_rate_probs = np.full(len(holdout), home_rate)
    home_rate_metrics = compute_metrics("naive_home_rate", y_holdout, home_rate_probs)
    ...
    gate_passes = model_metrics.log_loss < naive_ll
    ...
    manifest = {
        "track": "ufc_moneyline",
        "run_id": "train_ufc_baseline",
        "path": MODEL_ARTIFACT.relative_to(PROJECT_ROOT).as_posix(),
        "feature_set": FEATURE_SET,
        "model_version": "v1_logistic_platt",
        "promoted_at": datetime.now(timezone.utc).isoformat(),
    }
    ACTIVE_UFC_MANIFEST.write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    ...
    results: dict[str, Any] = {
        ...
        "phase_gate": {
            "rule": "Promote v1 if holdout log loss beats naive Elo / home rate.",
