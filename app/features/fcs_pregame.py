...
FEATURE_COLUMNS = ["elo_diff","srs_diff","season_win_pct_diff","last5_win_pct_diff","margin_diff","sos_diff","home_field","neutral_site","neutral_site_missing","conference_game","rank_diff","rank_missing"]
...
def build_features(games: pd.DataFrame) -> pd.DataFrame:
    ...
    for game_date,day in df.groupby("date",sort=True):
     pending=[]
     for g in day.itertuples(index=False):
        ...
        def sos(x):
            values=[]
            for entry in x:
                opp=[v for v in hist[entry[3]] if v[0]==season]
                if opp: values.append(winpct(opp))
            return sum(values)/len(values) if values else None
        hw,aw=winpct(hh),winpct(ah);hl,al=winpct(hh[-5:]),winpct(ah[-5:]);hm,am=margin(hh),margin(ah)
        hso,aso=sos(hh),sos(ah)
        hr=getattr(g,"home_rank",None);ar=getattr(g,"away_rank",None);rank_missing=int(pd.isna(hr) or pd.isna(ar))
        rows.append({"game_id":str(g.game_id),"date":g.date,"season":season,"home_team_id":h,"away_team_id":a,"home_win":int(g.home_win),"elo_diff":elo[h]-elo[a],"srs_diff":srs[h]-srs[a],"season_win_pct_diff":None if hw is None or aw is None else hw-aw,"last5_win_pct_diff":None if hl is None or al is None else hl-al,"margin_diff":None if hm is None or am is None else hm-am,"sos_diff":None if hso is None or aso is None else hso-aso,"home_field":0 if neutral else 1,"neutral_site":neutral,"neutral_site_missing":int(getattr(g,"neutral_site_missing",0) or 0),"conference_game":int(getattr(g,"conference_game",0) or 0),"rank_diff":None if rank_missing else float(ar)-float(hr),"rank_missing":rank_missing})
        pending.append((g,h,a,season,neutral))
     # Update only after every game on the date has been featurized. This makes
     # same-day doubleheaders/order changes incapable of leaking outcomes.
     for g,h,a,season,neutral in pending:
        ...
        hist[h].append((season,actual,hs-aws,a));hist[a].append((season,1-actual,aws-hs,h))
