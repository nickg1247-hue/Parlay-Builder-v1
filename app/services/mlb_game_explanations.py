...
def _collect_case(factors: list[dict[str, Any] | None], side: str) -> list[str]:
    key = f"{side}_text"
    out: list[str] = []
    for item in factors:
        if not item:
...
def build_mlb_game_explanation(
    ...
    *,
    ...
    why_home = _collect_case(factors, "home")
    why_away = _collect_case(factors, "away")
    comparison = [
        {
            "factor": f["factor"],
            "home": f["home"],
    ...
    return {
        ...
        "disclaimer": (
            "Rule-based summary of pregame inputs the model uses — not a full SHAP breakdown."
        ),
    }
