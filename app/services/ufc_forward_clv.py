"""Forward CLV logging for live UFC daily board +EV moneyline singles."""
...
from app.odds.ufc_odds_repository import fetch_ufc_from_api_if_allowed
...
FORWARD_CLV_UFC_LOG = PROJECT_ROOT / "data" / "processed" / "forward_clv_ufc_log.jsonl"
LIVE_ODDS_SOURCES = frozenset({"the_odds_api", "the_odds_api_live"})


def pick_id(board_date: str, fight_id: str, side: str) -> str:
    return f"ufc:{board_date}:{fight_id}:{side}"


def _read_all_rows() -> list[dict[str, Any]]:
    if not FORWARD_CLV_UFC_LOG.exists():
        return []
...
def _pick_label(pick: dict[str, Any], game: dict[str, Any]) -> str:
    return str(pick.get("fighter") or pick.get("team") or game.get("home_team") or "")


def log_live_picks(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """Log +EV moneyline singles from a fresh live UFC board payload."""
    ...
    for game in payload.get("slate") or []:
        ...
        pick = game["best_pick"]
        side = pick["side"]
        fid = str(game.get("fight_id") or game.get("game_id"))
        pid = pick_id(board_date, fid, side)
        ...
        row = {
            ...
            "commence_time": game.get("start_time_utc"),
            "close_status": None,
...
def _live_ufc_odds_df() -> pd.DataFrame:
    today = date.today()
    api_result = fetch_ufc_from_api_if_allowed(today)
    if api_result.denied or api_result.error or not api_result.events:
        return pd.DataFrame()
    rows = []
    for fight in api_result.events:
        rows.append(
            {
                "date_key": today.isoformat(),
                "home_team": normalize_fighter_name(fight.get("home_team", "")),
                "away_team": normalize_fighter_name(fight.get("away_team", "")),
                "home_key": fighter_match_key(fight.get("home_team", "")),
                "away_key": fighter_match_key(fight.get("away_team", "")),
...
def _match_odds_row(row: dict[str, Any], odds_df: pd.DataFrame) -> pd.Series | None:
    home_key = fighter_match_key(row["home_team"])
    away_key = fighter_match_key(row["away_team"])
    match = odds_df[
        (odds_df["date_key"] == row["board_date"])
        & (odds_df["home_key"] == home_key)
        & (odds_df["away_key"] == away_key)
    ]
    if not match.empty:
        return match.iloc[0]
    swapped = odds_df[
        (odds_df["date_key"] == row["board_date"])
        & (odds_df["home_key"] == away_key)
        & (odds_df["away_key"] == home_key)
    ]
...
def backfill_closing_odds(
    ...
    *,
    ...
    """Fetch quota-gated live UFC odds and fill close fields on open log rows."""
    rows = _read_all_rows()
    if not rows:
        return {"updated": 0, "missed": 0, "pending": 0, "sport": "ufc"}

    odds_df = _live_ufc_odds_df()
    if odds_df.empty:
        return {
            "updated": 0,
            "missed": 0,
    ...
    for _pid, row in latest.items():
        ...
        m = _match_odds_row(row, odds_df)
        commence = row.get("commence_time") or (m.get("commence_time") if m is not None else None)
        ...
        swapped = (
            fighter_match_key(row["home_team"]) == m["away_key"]
            and fighter_match_key(row["away_team"]) == m["home_key"]
        )
        if swapped:
        ...
        if fid in results_by_id and pd.notna(results_by_id[fid]):
...
def summarize_clv(days: int = 30) -> dict[str, Any]:
    """Aggregate UFC forward CLV log for API report."""
    ...
    try:
        ...
        log_path = FORWARD_CLV_UFC_LOG.as_posix()
