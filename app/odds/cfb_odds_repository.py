"""Persistent CFB odds snapshots — live API only (no historical bulk burn)."""
...
from app.odds.the_odds_api import fetch_live_cfb_odds

logger = logging.getLogger(__name__)

DEFAULT_REPO_DIR = PROJECT_ROOT / "data" / "processed" / "cfb_odds_repository"


def _repo_root() -> Path:
    import os

    override = os.getenv("CFB_ODDS_REPOSITORY_DIR", "").strip()
...
def load_date(game_date: date) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("Could not read CFB odds repository %s: %s", path, exc)
        return None
...
def normalize_cfb_events(
    events: list[dict[str, Any]],
    *,
    require_h2h: bool = True,
) -> list[dict[str, Any]]:
    """Parse The Odds API events into normalized CFB rows."""
...
def fetch_cfb_from_api_if_allowed(
    game_date: date,
    *,
    ...
    if game_date < date.today():
        return ApiFetchResult(denied=True, denied_reason="cfb_live_only_no_historical")
    ...
    try:
        events = fetch_live_cfb_odds(
            include_spreads=include_spreads,
            include_totals=include_totals,
        )
        normalized = normalize_cfb_events(events or [])
        return ApiFetchResult(events=normalized, source="the_odds_api_live")
    except Exception as exc:
        _release_quota_slot()
        logger.warning("CFB Odds API HTTP failed for %s: %s", game_date.isoformat(), exc)
        return ApiFetchResult(error=str(exc))


def get_cfb_odds_for_date(
    game_date: date,
    *,
    ...
    api_result = fetch_cfb_from_api_if_allowed(
        game_date,
        include_spreads=include_spreads,
        include_totals=include_totals,
    )
